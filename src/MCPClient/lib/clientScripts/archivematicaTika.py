...
tikaPath = "/usr/lib/archivematica/archivematicaCommon/externals/TIKA/tika-app-1.3.jar"

def getTikaVersion():
    command = "java -jar %s -V" % (tikaPath) 
...
def getTikaID(itemdirectoryPath):
    command = "java -jar %s -x -d %s" % (tikaPath, itemdirectoryPath) 
    exitCode, stdOut, stdErr = executeOrRun("command", command, printing=False)
